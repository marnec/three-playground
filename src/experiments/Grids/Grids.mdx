...
In my code I wrote the lerp factor as $1 - 0.0001^\delta$
...
In the *decay rate* form ($1 - e^{-k \Delta t}$) 
you specify how fast you converge towards $b$ by tweaking $k$, higher $k$ means faster movement.

In the *remaining fraction* form ($1 - \text{base}^{\Delta t}$) you specify what fraction of the distance remains after one second.
It's more intuitive for tweaking. With a base of `0.0001`, after one second only 0.01% of the
original distance remains, you're essentially there. If you wanted slower, smoother movement, you'd use a larger
